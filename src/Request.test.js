...
describe('Request', () => {
    ...
    describe('buildRequest', () => {
        ...
        it('sets request app, verb, url', async () => {
            const def = {
                url: 'someUrl',
                verb: 'get'
            };

            await Request.buildRequest('app', def);
        ...
        it('returns chain result of headers body', async () => {
            const def = {
                url: 'someUrl',
                verb: 'get',
                headers: 'headers',
            ...
            const request = await Request.buildRequest('app', def);
    ...
    describe('getUrl', () => {
        ...
        it('returns url value', async () => {
            expect(await Request.getUrl('url')).to.equal('url');
        });

        it('returns url callback value', async () => {
            expect(await Request.getUrl(() => 'url')).to.equal('url');
        });

        it('returns url async callback value', async () => {
            expect(await Request.getUrl(async () => 'url')).to.equal('url');
        });
    ...
    describe('getAuth', () => {
        ...
        it('returns auth value', async () => {
            expect(await Request.getAuth('authValue')).to.equal('authValue');
        });

        it('returns auth callback value', async () => {
            expect(await Request.getAuth(() => 'authValue')).to.equal('authValue');
        });

        it('returns auth callback value', async () => {
            expect(await Request.getAuth(async () => 'authValue')).to.equal('authValue');
        });
    ...
    describe('applyBody', () => {
        ...
        it('returns request when body is undefined', async () => {
            expect(await Request.applyBody('req', undefined)).to.equal('req');
        });

        it('calls request.send(body)', async () => {
            const req = {
                    send: sinon.spy()
                },
                body = 'bodyStub';

            await Request.applyBody(req, body);

            expect(req.send.calledOnce).to.be.true;
            expect(req.send.calledWithExactly(body)).to.be.true;
        });

        it('calls request.send(body()) when body is a function', async () => {
            const req = {
                    send: sinon.spy()
                },
                body = () => 'bodyStub';
        ...
        it('calls request.send(body()) when body is a async function', async () => {
            const req = {
                    send: sinon.spy()
                },
                body = async () => 'bodyStub';
    ...
    describe('processRequest', () => {
        ...
        it('calls make and request.test()', async () => {
            await Request.processRequest('app', def, config);
    ...
    describe('make', () => {
        ...
        it('returns instance of Request', async () => {
            expect(await Request.make('app', {})).to.be.instanceOf(Request);
        });
    ...
    describe('test', () => {
        ...
        it('returns call to expect', async () => {
            req = await (new Request('app', {expect: 'value'})).build();

            expect(req.test()).to.equal(req.request);
        });

        it('calls expect with value', async () => {
            req = await (new Request('app', {
                expect: 'value'
            })).build();
            req.test();
        ...
        it('calls expect with body value', async () => {
            req = await (new Request('app', {
                expect: {foo: 'bar'}
            })).build();
            req.test();
        ...
        it('calls expect with def.expect.body', async () => {
            req = await (new Request('app', {
                expect: {
                    body: {foo: 'bar'}
                }
        ...
        it('calls expect with def.expect.headers', async () => {
            req = await (new Request('app', {
                expect: {
                    headers: {
                        foo: 1,
        ...
        it('calls expect(status) with headers.status or Status-Code', async () => {
            req = await (new Request('app', {
                expect: {
                    headers: {
                        'Status-Code': 456,
        ...
        it('calls expect with headers and body', async () => {
            req = await (new Request('app', {
                expect: {
                    headers: {foo: 1},
                    body: {bar: 2}
        ...
        it('registers _onTestError as catch', async () => {
            req = await (new Request('app', {expect: {}})).build();
            req.test();
        ...
        it('registers _response on first expect call', async () => {
            req = await (new Request('app', {expect: {}})).build();
    ...
    describe('_onTestError', () => {
        ...
        it('calls definition.error when provided', async () => {
            def = {error: sinon.spy()};
            req = await(new Request('app', def)).build();
            req._response = 'resp';
        ...
        it('does not throw when definition.error is not provided', async () => {
            def = {};
            req = await (new Request('app', def)).build();
            req._response = 'resp';
    ...
    describe('_expect', () => {
        ...
        it('calls request.expect', async () => {
            req = await (new Request('app', {})).build();
        ...
        it('sets request.expect() value to this._request', async () => {
            req = await (new Request('app', {})).build();
