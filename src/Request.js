...
const callbackOrValue = async (o) => (typeof o === 'function') ? await o() : o; // FIXME use always
...
class Request {
    ...
    static async processRequestDefinition(app, definition, config) {
        ...
        if (!definition.auth) {
            ...
            return await Request.processRequest(app, definition, config);
        } else if (Array.isArray(definition.auth)) {
        ...
        return await Request.processAuthenticatedRequest(app, definition, config);
    }

    // entry point
    static async process(app, definition, config) {
        if (definition.steps) {
            return definition.steps.reduce((previousRequest, stepDef, index) => {
                return previousRequest.then(async response => {
                    debug(`Processing step ${index}`);
                    stepDef = typeof stepDef === 'function' ? stepDef(response) : stepDef;

                    return await Request.processRequestDefinition(app, {
                        ...definition,
                        ...stepDef
                    }, config);
                });
